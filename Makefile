...
AGENTS_COLOR := $(FONT_BRIGHT_BLUE)  # automagik_flashed: Bright Blue (cyan blue)
...
AM_AGENTS_LABS_DIR := $(SERVICES_DIR)/automagik_flashed
...
SERVICES := automagik_flashed automagik-spark automagik-tools automagik-omni automagik-ui

# Actual runnable services (excludes automagik-tools which is a library)
RUNNABLE_SERVICES := automagik_flashed automagik-spark automagik-omni automagik-ui

# PM2 service names
PM2_SERVICES := automagik_flashed automagik-spark-api automagik-spark-worker automagik-tools-sse automagik-tools-http automagik-omni automagik-ui

# Repository URLs
AM_AGENTS_LABS_URL := https://github.com/namastexlabs/automagik_flashed.git
...
uninstall-infrastructure: ## 🗑️ Uninstall Docker infrastructure (remove containers, images, volumes)
	...
	@# Remove automagik_flashed docker project
	@if [ -f "automagik_flashed/docker/docker-compose.yml" ]; then \
		$(DOCKER_COMPOSE) -f automagik_flashed/docker/docker-compose.yml -p docker down -v --rmi all --remove-orphans >/dev/null 2>&1 || true; \
	fi
...
setup-env-files: ## 📝 Setup .env files from templates for main and all services
	...
	@for service in "automagik-spark" "automagik-tools" "automagik-omni"; do \
	...
	@# Note: automagik_flashed reads the main .env file directly (no separate .env needed)
	@if [ -d "$(AM_AGENTS_LABS_DIR)" ]; then \
		echo -e "$(FONT_GREEN)$(CHECKMARK) automagik_flashed uses parent .env file$(FONT_RESET)"; \
	fi
...
install-dependencies-only: ## 📦 Install only dependencies (no service registration)
	...
	@if [ -d "$(AM_AGENTS_LABS_DIR)" ]; then \
		echo -e "$(FONT_CYAN)$(INFO) Installing dependencies for automagik_flashed...$(FONT_RESET)"; \
...
install-agents: ## Install automagik_flashed service
	$(call print_status,Installing $(AGENTS_COLOR)automagik_flashed$(FONT_RESET) service...)
	@if [ ! -d "$(AM_AGENTS_LABS_DIR)" ]; then \
		$(call ensure_repository,automagik_flashed,$(AM_AGENTS_LABS_DIR),$(AM_AGENTS_LABS_URL)); \
	fi
...
status-all: ## 📊 Check status of all services
	...
	@pm2 list | sed -E \
		-e 's/(automagik_flashed)/\x1b[94m\1\x1b[0m/g' \
...
start-agents: ## 🚀 Start automagik_flashed service only
	$(call print_status,Starting $(AGENTS_COLOR)automagik_flashed$(FONT_RESET) service...)
	@cd $(AM_AGENTS_LABS_DIR) && pm2 start ecosystem.config.js
...
stop-agents: ## 🛑 Stop automagik_flashed service only
	$(call print_status,Stopping $(AGENTS_COLOR)automagik_flashed$(FONT_RESET) service...)
	@pm2 stop automagik_flashed 2>/dev/null || true
...
restart-agents: ## 🔄 Restart automagik_flashed service only
	$(call print_status,Restarting $(AGENTS_COLOR)automagik_flashed$(FONT_RESET) service...)
	@pm2 restart automagik_flashed 2>/dev/null || (cd $(AM_AGENTS_LABS_DIR) && pm2 start ecosystem.config.js)
...
status-agents: ## 📊 Check automagik_flashed status only
	$(call print_status,Checking $(AGENTS_COLOR)automagik_flashed$(FONT_RESET) status...)
	@pm2 show automagik_flashed 2>/dev/null || echo "Service not found"
...
logs-agents: ## 📋 Follow automagik_flashed logs
	$(call print_status,Following $(AGENTS_COLOR)automagik_flashed$(FONT_RESET) logs...)
	@pm2 logs automagik_flashed
...
install: ## 🚀 Install Automagik suite (infrastructure + services - no auto-start)
	...
	@if docker ps --filter "name=automagik_flashed-postgres" --filter "status=running" --format "{{.Names}}" | grep -q "automagik_flashed-postgres" && \
		...
		for repo in $$updated_repos; do \
			case $$repo in \
				"main") \
					...
					$(MAKE) restart ;; \
				"automagik_flashed") $(MAKE) restart-agents ;; \
...
pull-agents: ## 🔄 Pull automagik_flashed repository only
	$(call git_pull_service,automagik_flashed,$(AGENTS_COLOR),$(AM_AGENTS_LABS_DIR),$(AM_AGENTS_LABS_URL))
...
logs: ## 📋 Show logs from all services (N=lines F=1 for follow mode)
	...
	@if [ "$(F)" = "1" ]; then \
		...
		pm2 logs | sed -E \
			-e 's/(automagik_flashed)/$(AGENTS_COLOR)\1$(FONT_RESET)/g' \
		...
		pm2 logs automagik_flashed --lines $(N) --nostream 2>/dev/null | sed "s/^/$(AGENTS_COLOR)  $(FONT_RESET)/" || echo -e "$(FONT_RED)  Service not found$(FONT_RESET)"; \
